...
export class D3GraphComponent implements OnInit, OnChanges {
    ...
    */
    ...
    createBarGraph(): void {
        ...
        let svg = d3.select(_this.el.nativeElement).append('svg').attr({ width: _this.width, height: _this.height });
        // Bars
    ...
    createLineChart(): void {
        ...
        let xAxisGen = d3.svg.axis().scale(xScale).orient('bottom');
        if(isDate) {
            xAxisGen.tickFormat(d3.time.format('%b'));
        }
        let yAxisGen = d3.svg.axis().scale(yScale).orient('left');
    ...
    createPieChart(): void {
        let _this = this;
        let isDate: boolean = (this.xType==='date');
        let timeFormat = d3.time.format('%b');
        let r = _this.width/2;
        let color = d3.scale.category20c();
        let vis = d3.select(_this.el.nativeElement).append('svg:svg').data([_this.data])
            .attr({ width: r*2, height: r*2 })
            .append('svg:g')
            .attr('transform', 'translate(' + r + ',' + r + ')');
        let pie = d3.layout.pie().value(function(d: any) { return d[_this.yKey]; });
